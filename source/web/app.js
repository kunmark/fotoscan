...
function addScan(rawValue, options = {}) {
  ...
  setStatus(`已记录 ${barcode} (${type})${duplicateMessage}`, "success");
...
function handleCameraDetection(rawValue) {
  ...
  addScan(barcode, { focusInput: false });
  setCameraStatus(`摄像头已识别 ${barcode}，记录已写入。`, "success");
  return true;
}
